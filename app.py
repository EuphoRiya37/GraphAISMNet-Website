...
.navbar{background:linear-gradient(135deg,#0D2E6E 0%,#1B4FA8 55%,#0D6B4F 100%);
    padding:0 2.5rem;display:flex;align-items:center;justify-content:space-between;
    height:68px;box-shadow:0 3px 20px rgba(13,46,110,.35);position:sticky;top:0;z-index:1000;}
...
.nav-btn{color:rgba(255,255,255,.8);background:transparent;border:none;padding:.42rem .95rem;
    border-radius:6px;font-size:.86rem;font-weight:500;text-decoration:none;
    display:inline-block;transition:all .2s;font-family:'Inter';}
...
.hero-banner{background:linear-gradient(135deg,#0D2E6E 0%,#1B4FA8 60%,#0D6B4F 100%);
...
.hero-title{font-family:'Poppins',sans-serif;font-weight:800;font-size:2.4rem;letter-spacing:-.02em;margin-bottom:.4rem;}
...
.section-heading{font-family:'Poppins',sans-serif;font-weight:700;font-size:1.5rem;color:#0D2E6E;margin-bottom:.3rem;}
...
.stButton>button{background:linear-gradient(135deg,#1B4FA8 0%,#0D7A5F 100%)!important;
...
.stTextArea textarea,.stTextInput input{background:white!important;border:1.5px solid #D5E0F5!important;
...
.stTextArea textarea:focus,.stTextInput input:focus{border-color:#1B4FA8!important;
    box-shadow:0 0 0 3px rgba(27,79,168,.12)!important;}
...
@media screen and (max-width:768px){
    .navbar{flex-direction:column;height:auto;padding:1rem;gap:.8rem;}
    .navbar-links{width:100%;overflow-x:auto;white-space:nowrap;padding-bottom:.5rem;}
...
# ── GAT Model (only model used — both random and scaffold) ────────────────────
...
# ── Load single GAT model ──────────────────────────────────────────────────────
@st.cache_resource
def load_model():
    gat=SimpleGAT(); gat_ok=False
    if os.path.exists("model_gat.pt"):
        gat.load_state_dict(torch.load("model_gat.pt",map_location="cpu")); gat_ok=True
    gat.eval()
    return gat, gat_ok

gat_model, gat_loaded = load_model()

def run_predict(smiles):
...
def on_example_change(smi_key, ex_key):
    ...
    st.session_state[smi_key]=EXAMPLES[ex] if ex!="— Custom input —" else ""

def predict_panel(page_label, btn_key, smi_key):
    left,right=st.columns([1,1.2],gap="large")
    with left:
        st.markdown('<div class="card">',unsafe_allow_html=True)
        st.markdown('<div style="font-family:Poppins;font-weight:600;font-size:1rem;color:#0D2E6E;margin-bottom:.8rem;">\U0001f52c Single Molecule Input</div>',unsafe_allow_html=True)
        st.selectbox("Load example",["— Custom input —"]+list(EXAMPLES.keys()),
                     key=smi_key+"_ex",on_change=on_example_change,args=(smi_key,smi_key+"_ex"))
        smi=st.text_area("SMILES string",height=90,placeholder="Paste SMILES here\u2026",key=smi_key)
        run=st.button(f"\u25b6  Run GAT Prediction",key=btn_key)
        if smi.strip():
...
<div class="navbar">
  <div class="navbar-brand">
    <div class="navbar-logo">\U0001f9ec</div>
    <div>
...
if PAGE == "home":
    ...
    <div class="hero-banner">
        ...
        <div class="hero-badges">
            ...
            <span class="hero-badge">GAT \u2014 Best Model for Both Splits</span>
    ...
    with col_center:
        ...
        <div style="text-align:center;margin-bottom:1rem;">
            <div style="font-size:2rem;">\U0001f9ec</div>
            <div style="font-family:Poppins;font-weight:700;font-size:1.1rem;color:#0D2E6E;margin:.3rem 0;">Welcome to GraphAISMNet</div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;margin-top:1.2rem;">
            <div style="background:#EEF4FF;border-radius:8px;padding:.7rem;text-align:center;">
                <div style="font-size:1.2rem;">\U0001f916</div>
            ...
            <div style="background:#E8F8F2;border-radius:8px;padding:.7rem;text-align:center;">
                <div style="font-size:1.2rem;">\U0001f3af</div>
            ...
            <div style="background:#FFF8E6;border-radius:8px;padding:.7rem;text-align:center;">
                <div style="font-size:1.2rem;">\U0001f513</div>
    ...
    with col_right:
        ...
        <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.8rem;">
            <span style="font-size:1.3rem;">\U0001f680</span>
        ...
        split_choice=st.selectbox("Prediction split",["Random Split \u2014 GAT (Recommended)","Scaffold Split \u2014 GAT"],key="home_model")
        ...
        for icon,title,sub,link in [
            ("\U0001f4d6","How to Use","Step-by-step guide","?page=help"),
            ("\u2328\ufe0f","Input Format","Accepted SMILES format","?page=help"),
            ("\U0001f4ca","Understanding Results","Interpreting prediction scores","?page=help"),
            ("\U0001f52c","Algorithm","Model architecture","?page=algorithm"),
        ...
        <div style="padding:0 2.5rem 1rem;">
            <div style="background:white;border-radius:14px;padding:1.5rem 2rem;
                ...
                <div style="font-family:Poppins;font-weight:700;font-size:1.1rem;
                     color:#0D2E6E;margin-bottom:1rem;">\U0001f4d0 GraphAISMNet \u2014 Methodology Workflow</div>
        """, unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-sub">GAT \u2014 Best model across both split conditions \u2014 5-fold cross-validation on test set</div>', unsafe_allow_html=True)
    c1,c2=st.columns(2,gap="medium")
    with c1:
        st.markdown("""<div class="card card-blue">
        <div style="font-family:Poppins;font-weight:700;color:#1B4FA8;font-size:.95rem;margin-bottom:.3rem;">\U0001f535 Random Split \u2014 GAT (Graph Attention Network)</div>
        ...
        for v,l in [("0.951","AUC-ROC"),("89.2%","Accuracy"),("0.785","MCC"),("0.718","R\u00b2")]:
    ...
    with c2:
        ...
        <div style="font-family:Poppins;font-weight:700;color:#0D7A5F;font-size:.95rem;margin-bottom:.3rem;">\U0001f7e2 Scaffold Split \u2014 GAT (Graph Attention Network)</div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f535 Random Split Prediction</div><div class="hero-sub" style="margin-bottom:.5rem;">Model: Graph Attention Network (GAT) \u00b7 AUC=0.951 \u00b7 ACC=89.2%</div><div class="hero-badges"><span class="hero-badge green">\u2713 Recommended for general use</span><span class="hero-badge">Random stratified split</span><span class="hero-badge">With SMILES augmentation</span></div></div>', unsafe_allow_html=True)
    if not gat_loaded:
        st.markdown('<div style="padding:1rem 2.5rem 0;"><div class="warn-box">\u26a0\ufe0f model_gat.pt not found \u2014 running in demo mode.</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    predict_panel("random","r_run","r_smi")
    st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f7e2 Scaffold Split Prediction</div><div class="hero-sub" style="margin-bottom:.5rem;">Model: Graph Attention Network (GAT) \u00b7 AUC=0.928 \u00b7 ACC=86.9%</div><div class="hero-badges"><span class="hero-badge">Murcko scaffold split</span><span class="hero-badge green">\u2713 More rigorous generalisation</span></div></div>', unsafe_allow_html=True)
    if not gat_loaded:
        st.markdown('<div style="padding:1rem 2.5rem 0;"><div class="warn-box">\u26a0\ufe0f model_gat.pt not found \u2014 running in demo mode.</div></div>', unsafe_allow_html=True)
    st.markdown('<div style="padding:.5rem 2.5rem 0;"><div class="info-box">ℹ️ Scaffold split tests on structurally novel chemical series not seen during training, providing a more rigorous assessment of generalisation.</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    predict_panel("scaffold","s_run","s_smi")
    st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f4cb Batch Prediction</div><div class="hero-sub">High-throughput screening \u2014 GAT model</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    tab1,tab2=st.tabs(["  \U0001f4c1  Upload CSV  ","  \u270f\ufe0f  Paste SMILES  "])
    with tab1:
        ...
        if uploaded:
            ...
            if not smi_col:
                st.markdown('<div class="warn-box">\u26a0\ufe0f No SMILES column found</div>',unsafe_allow_html=True)
            else:
                if st.button(f"\u25b6  Predict all {len(df_in)} molecules"):
                    prog=st.progress(0); results=[]
                    for i,smi in enumerate(df_in[smi_col].astype(str)):
                        r=run_predict(smi)
                        results.append({"Activity":"Active" if r and r["active"] else "Inactive",
                                        "Probability":r["prob"] if r else None,
                                        "pIC50":r["pic50"] if r else None,
                                        "IC50_uM":r["ic50"] if r else None})
        ...
        if st.button("\u25b6  Predict Pasted SMILES"):
            ...
            if lines:
                with st.spinner(f"Predicting {len(lines)} molecules\u2026"):
                    res=[{"SMILES":s,
                          "Activity":"Active" if (r:=run_predict(s)) and r["active"] else "Inactive",
                          "Probability":r["prob"] if r else None,
                          "pIC50":r["pic50"] if r else None,
                          "IC50_uM":r["ic50"] if r else None} for s in lines]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f4d0 Algorithm & Methodology</div><div class="hero-sub">GraphAISMNet \u2014 Graph Neural Network Framework</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    if os.path.exists("GRAPH.png"):
    ...
    for num,title,color,desc in [
        ...
        ("8","Performance","#0D7A5F","Best model: GAT across both splits \u2014 Random split (AUC=0.951, ACC=89.2%), Scaffold split (AUC=0.928, ACC=86.9%). External validation on FDA-approved drugs."),
    ]:
    ...
    def mk_table(rows, best):
        ...
        for r in rows:
            ...
            mc='<td class="model-col">'+r[0]+star+"</td>"
            dc="".join('<td class="best">'+v+"</td>" if is_best else "<td>"+v+"</td>" for v in r[1:])
            body+="<tr>"+mc+dc+"</tr>"
    ...
    with tab_r:
        ...
        st.markdown(mk_table([["GCN","0.944","0.880","0.760","0.903","0.856","0.880","0.698","0.668"],["GAT","0.951","0.892","0.785","0.902","0.882","0.892","0.718","0.644"],["GT","0.950","0.894","0.789","0.909","0.880","0.894","0.712","0.651"],["MSMP","0.951","0.890","0.781","0.888","0.893","0.890","0.709","0.654"]],"GAT"),unsafe_allow_html=True)
    with tab_s:
        st.markdown("<b style='font-size:.82rem;color:#6B7A99;'>No Augmentation</b>",unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f4c2 Dataset</div><div class="hero-sub">Training and evaluation dataset details</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    ...
    st.markdown('<div class="info-box" style="margin-top:1rem;">\U0001f4e5 To download the training dataset, please contact the development team.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for title,color,content in [
        ("\U0001f4d6 How to Use","#1B4FA8","<b>Step 1:</b> Click <b>Predict (Random)</b> in the navbar (recommended).<br><b>Step 2:</b> Enter a SMILES string or select an example molecule.<br><b>Step 3:</b> Click Run GAT Prediction \u2192 get Activity, pIC\u2085\u2080, IC\u2085\u2080, and properties.<br><b>Step 4:</b> For multiple molecules, use the <b>Batch</b> page."),
        ...
        ("\U0001f4ca Understanding Results","#7B3FA8","<b>Activity:</b> Active (pIC\u2085\u2080 \u2265 6.0) or Inactive<br><b>Confidence:</b> Sigmoid probability 0\u20131<br><b>pIC\u2085\u2080:</b> Higher = more potent<br><b>IC\u2085\u2080 (\u00b5M):</b> 10^(6 \u2212 pIC\u2085\u2080)<br><br><b>Which split?</b> Random Split is recommended for general use. Scaffold Split gives more conservative estimates for novel chemical series."),
        ("\u2753 FAQ","#0D2E6E","<b>Q: Is this free?</b> Yes, completely free for research.<br><b>Q: Can I use this clinically?</b> No \u2014 research use only.<br><b>Q: Why is scaffold split accuracy lower?</b> It tests on structurally novel compounds not seen during training \u2014 a harder, more realistic evaluation.<br><b>Q: Same model for both splits?</b> Yes \u2014 GAT is used for both. The difference is the evaluation protocol, not the model."),
    ]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="hero-banner" style="padding:1.5rem 3rem;"><div class="hero-title" style="font-size:1.8rem;">\U0001f4ec Contact Us</div><div class="hero-sub">Get in touch with the GraphAISMNet team</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-section">', unsafe_allow_html=True)
    ...
    with c1:
        ...
        <div style="font-size:.87rem;color:#2D3748;line-height:1.8;">
            Research Scholar<br>Computational Lab<br>
            Department of Genetic Engineering<br>School of Bioengineering<br>SRMIST
        </div></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="card card-blue">
    ...
    with c2:
        ...
        <div style="font-size:.87rem;color:#2D3748;line-height:1.8;">
            Associate Professor<br>Computational Lab<br>
            Department of Computational Intelligence<br>School of Computing<br>SRMIST
        </div></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="card card-blue" style="margin-bottom:1.2rem;">
        ...
        st.markdown("""
        <div style="margin-top:.5rem;background:#E8F8F2;border-radius:12px;padding:1.2rem;">
            <div style="font-family:Poppins;font-weight:700;color:#0D7A5F;font-size:.9rem;margin-bottom:.5rem;">Citation</div>
            <div style="background:#F7FAFF;border-radius:8px;padding:.8rem;font-family:JetBrains Mono;font-size:.75rem;line-height:1.7;color:#2D3748;border:1px solid #E0E8F5;">
Riya et al. (2025). GraphAISMNet: GNN-Based Multi-Task Framework for Anti-Inflammatory Molecular Activity Prediction. SRMIST, Chennai.
            </div>
            <div style="margin-top:.8rem;font-size:.82rem;color:#4A5568;">
                <b style="color:#0D7A5F;">Disclaimer:</b> Computational research tool only. Predictions require experimental validation.
            </div>
        </div>""", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
